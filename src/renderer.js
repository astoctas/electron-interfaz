...
wss.on('connection', function connection(ws) {
  ws.on('message', function incoming(message) {
    ...
    var key = message.key || false;
    var data = message.data || false;
    switch(key) {
      case 'ANALOG': 
      ifaz.lcd().clearTimeout();
      var res = ifaz.analog(data.index)[data.method](function (result) {
        console.log(result)
        //var msg = JSON.stringify({"key": "ANALOG_MESSAGE"});
        var msg = "ANALOG_MESSAGE";
        ws.send(msg);
      });
